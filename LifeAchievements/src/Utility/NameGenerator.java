...
public class NameGenerator {
	...
	private String vowelsCAP = "AEIOUYAÖÜ";
	...
	private String generateVowel(boolean caps) {
		if (caps) {
			if (doubleBool) {
				...
				doubleChar = "" + vowels.charAt(r.nextInt(vowels.length()))
						+ trippleChar;
			}
			return vowelStart + vowelsCAP.charAt(r.nextInt(vowelsCAP.length()))
					+ doubleChar + vowelEnd;
		} else {
			if (doubleBool) {
				if (trippleBool) {
			...
			return vowelStart + vowels.charAt(r.nextInt(vowels.length()))
					+ doubleChar + vowelEnd;
		}
	}
	...
	private String generateConsonant(boolean caps) {
		if (caps) {
			if (doubleBool) {
				if (trippleBool) {
					trippleChar = ""
							+ consonants.charAt(r.nextInt(consonants.length()));
				}
				doubleChar = ""
						+ consonants.charAt(r.nextInt(consonants.length()))
						+ trippleChar;
			...
			return consonantStart
					+ consonantsCAP.charAt(r.nextInt(consonantsCAP.length()))
					+ doubleChar + consonantEnd;
		} else {
			if (doubleBool) {
				if (trippleBool) {
					trippleChar = ""
