...
public class AccessController {
    ...
    private static User loggedInAs = new User(Permission.GUEST);
    ...
    public static void getStatus() {
        ...
        if (currentPerson != null) {
            System.out.println("Person in use: " + currentPerson.getFirstName()
    ...
    public static void selectPerson(int id) {
        if (Permission.getPermission("AccessController.selectPerson")) {
            List<Person> persons = PersonDatabase.getPersons(getAdmin()
                    .getPerson());
            boolean found = false;
    ...
    protected static boolean login() throws IOException {
        ...
        if (loggedInAsUser || loggedInAsAdmin) {
            ...
            loggedInAs = new User(Permission.GUEST);
        }
        ...
        if (loggedOut) {
            ...
            if (loginUser.equals(admin) && loginPassword.equals(password)) {
                ...
                loggedInAs = new User(Permission.ADMIN);
                loggedInAsAdmin = true;
                authentic = true;
            } else if (isUser(loginUser, loginPassword)) {
                System.out.println("Login success.");
                loggedInAs = new User(Permission.USER);
                loggedInAsUser = true;
                authentic = true;
            } else {
            ...
            if (loggedInAs != null) {
                if (loggedInAs.getPerson() != null) {
                    System.out.println(">>>> " + loggedInAs.toString() + ", "
                            + loggedInAs.getPermission());
                }
            }
        }
        return authentic;
    ...
    protected static User getAdmin() {
        // TODO log this event
        return new User(Permission.ADMIN);
    }
